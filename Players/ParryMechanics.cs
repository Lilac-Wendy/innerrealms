...
using Terraria.Localization; // Added for localization

namespace Taoism.Players
{
    public class ParryMechanics : ModPlayer
    {
        private const int ParryTimerToleranceTicks = 180;
        private const float DefaultAllowedRadius = 64f;
        private const float StaggerHealthThreshold = 0.1f;
        private const int CooldownDurationTicks = 180;
        private const string LocalizationPath = "Mods.taoism.";
        string GetLocalizedText(string key, params object[] args)
        {
            return Language.GetTextValue(LocalizationPath + key, args);
        }
        ...
        {
            if (parryActive || isOnCooldown) return;
            isOnCooldown = true;
            comboCount = 0;
            cooldownTimer = 0;  // Make this Negative if you want longer cooldowns after taking damage.
            Reset();
            CombatText.NewText(
                Player.Hitbox,
                Color.OrangeRed,
                GetLocalizedText("ParryMechanics.Messages.ParryDisabled"),
                dramatic: true
            );
                
            Dust.NewDustPerfect(Player.Center, DustID.Smoke, Vector2.Zero, 100, Color.Gray, 1.5f);
            SoundEngine.PlaySound(SoundID.Item10 with { Pitch = -0.7f }, Player.Center);
        }
        ...
        
        {
            if (isOnCooldown)
            {
                CombatText.NewText(
                    Player.Hitbox,
                    Color.Orange,
                    GetLocalizedText("ParryMechanics.Messages.ParryOnCooldown"),
                    dramatic: true
                );
                return;
            }
            ...
            if (lastParryDistance > allowedRadius)
            {
                if (Main.rand.NextFloat() < 0.5f)
                {
                    CombatText.NewText(
                        Player.Hitbox,
                        Color.OrangeRed,
                        GetLocalizedText("ParryMechanics.Messages.ParryFailed"),
                        dramatic: true
                    );
                    return;
                }
                ...
                CombatText.NewText(
                    Player.Hitbox,
                    Color.Yellow,
                    GetLocalizedText("ParryMechanics.Messages.ParrySucceeded"),
                    dramatic: true
                );
            }
            ...
            CombatText.NewText(
                Player.Hitbox,
                Color.LightGreen,
                GetLocalizedText("ParryMechanics.Messages.ParryPossible"),
                dramatic: true
            );
        }

        public void CheckParryInput(Player player, Vector2 parryGaugeWorldPosition,
        ...
        public void Update(Player player, Vector2 parryGaugeWorldPosition, float allowedRadius = DefaultAllowedRadius)
        {
            ...
            if (distance > allowedRadius)
            {
                CombatText.NewText(
                    Player.Hitbox,
                    Color.Red,
                    GetLocalizedText("ParryMechanics.Messages.ParryNotPossible"),
                    dramatic: true
                );
                Reset();
                return;
            ...
            if (parryTimer >= ParryTimerToleranceTicks)
            {
                CombatText.NewText(
                    Player.Hitbox,
                    Color.Gray,
                    GetLocalizedText("ParryMechanics.Messages.ParryTimeout"),
                    dramatic: true
                );
                Reset();
            }
            ...
            if (isOnCooldown)
            {
                ...
                if (cooldownTimer >= CooldownDurationTicks)
                {
                    ...
                    CombatText.NewText(
                        Player.Hitbox,
                        Color.LightGreen,
                        GetLocalizedText("ParryMechanics.Messages.ParryCooldownEnded"),
                        dramatic: true
                    );
                }
            }
        ...
        private void ExecuteElementalEffect(Player player, float currentDistance, float maxDistance)
        {
            ...
            {
                comboCount = 0;
                if (gauge != null) gauge.EarthFrameIndex = 0;
            }
            ...
            if (isValidWeapon)
            {
                ...
                CombatText.NewText(player.Hitbox, Color.Gold, 
                    GetLocalizedText("ParryMechanics.Messages.ComboCount", comboCount), 
                    dramatic: true);
        ...
        private void ExecuteWaterUppercut(Player player, int damage)
        {
            ...
            trackedTarget.AddBuff(BuffID.Slow, 540);
            CombatText.NewText(trackedTarget.Hitbox, Color.Cyan, 
                GetLocalizedText("ParryMechanics.Messages.WaterUppercut"), 
                dramatic: false);
        ...
        private void ExecuteWoodenJab(Player player, int damage)
        {
            ...
            if (trackedTarget.knockBackResist == 0f)
            {
                trackedTarget.velocity += direction * 5f;
            }
            else
            {
                var hitInfo = new NPC.HitInfo()
            ...
            CombatText.NewText(trackedTarget.Hitbox, Color.LimeGreen, 
                GetLocalizedText("ParryMechanics.Messages.WoodenJab"),
                dramatic: false);
        ...
        private void ExecuteFieryLunge(Player player, int damage)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                Dust dust = Dust.NewDustDirect(player.position, player.width, player.height, DustID.IchorTorch, 0f, 0f,
                    100, default, 2f);
                dust.noGravity = true;
                dust.velocity *= 2f;
            }
            CombatText.NewText(trackedTarget.Hitbox, Color.OrangeRed, 
                GetLocalizedText("ParryMechanics.Messages.FieryLunge"),
                dramatic: false);
            player.AddBuff(BuffID.WeaponImbueIchor, 300);
            damage = (int)(damage * 1.3f);
        ...
        private void ExecuteEarthenSweep(Player player, int damage)
        {
            ...
            CombatText.NewText(trackedTarget.Hitbox, new Color(150, 75, 0),  // Brownish color
                GetLocalizedText("ParryMechanics.Messages.EarthenSweep"),
                dramatic: false);
        ...
        private void ExecuteIronSurge(Player player, int damage)
        {
            ...
            CombatText.NewText(trackedTarget.Hitbox, Color.Silver, 
                GetLocalizedText("ParryMechanics.Messages.IronSurge"),
                dramatic: true); 
        }

        private void ApplyChainDamage(Player player, NPC currentTarget, int damage, HashSet<int> hitNpcs,
            int chainCount)
        {
            ...
            Dust.NewDust(currentTarget.position, currentTarget.width, currentTarget.height, DustID.Electric, 0f, 0f, 0,
                default, 1.5f);
        ...
        public void StaggerEnemy(Player player, int baseDamage)
        {
            ...
            CombatText.NewText(trackedTarget.Hitbox, Color.Gold, 
                GetLocalizedText("ParryMechanics.Messages.STAGGERED"), 
                dramatic: true);
            SoundEngine.PlaySound(SoundID.Item89, trackedTarget.position);
            Main.NewText(GetLocalizedText("ParryMechanics.Messages.FinishingBlow"), Color.Gold);
        }
